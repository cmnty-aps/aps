...
async function makeSticker(m: any, sock: any, deviceId: string) {
  ...
  try {
    ...
    const sticker = new Sticker(buffer, {
      pack: deviceConfig.stickerPack || "CMNTY-BOT",
      author: deviceConfig.stickerAuthor || "@OJI",
...
async function donateTako(m: any, sock: any, q: string, deviceId: string) {
  ...
  try {
      const apikey = config.neoxrApiKey || 'Milik-Bot-OurinMD';
...
async function qcSticker(m: any, sock: any, args: string[], deviceId: string) {
    ...
    try {
        ...
        const sticker = new Sticker(buffer, {
            pack: deviceConfig.stickerPack || "CMNTY-BOT",
            author: deviceConfig.stickerAuthor || "Bot",
            type: StickerTypes.FULL,
            id: "QC-" + Date.now(),
            quality: 100,
        });
  ...
  if (!qMsg?.stickerMessage) {
    return sock.sendMessage(jid, {
      text: `🖼️ *sᴛɪᴄᴋᴇʀ ᴡᴀᴛᴇʀᴍᴀʀᴋ*\n\n` +
            ...
            `> \`${prefix}swm packname\`\n\n` +
            `*ᴄᴏɴᴛᴏʜ:*\n` +
            `> \`${prefix}swm CMNTY-BOT\`\n` +
            `> \`${prefix}swm CMNTY-BOT|@OJI\` _(packname + author)_`,
      contextInfo: getContextInfo(deviceConfig, m)
    }, { quoted: m });
  }
  ...
  if (!input) {
    return sock.sendMessage(jid, {
      text: `❌ *ɢᴀɢᴀʟ*\n\n` +
            ...
            `*ᴄᴏɴᴛᴏʜ:*\n` +
            `> \`${prefix}swm CMNTY-BOT\`\n` +
            `> \`${prefix}swm CMNTY-BOT|@OJI\` _(+ author)_`,
      contextInfo: getContextInfo(deviceConfig, m)
    }, { quoted: m });
  }
...
async function pinpackMode(m: any, sock: any, q: string, deviceId: string) {
  ...
  try {
    ...
    const author = deviceConfig.stickerAuthor || "@OJI";
...
async function stickerPackHandler(m: any, sock: any, query: string, deviceId: string) {
  ...
  try {
    ...
    const packname = randPick.name || deviceConfig.stickerPack || "CMNTY-BOT";
    const author = deviceConfig.stickerAuthor || "@OJI";
...
async function connectToWhatsApp(
  ...
  sock.ev.on("chats.delete", async (deletions) => {
    const channelId = instance.config?.autoFollowChannelId || config.channel.autoFollowId;
    if (channelId && deletions.includes(channelId)) {
      console.log(`[NEWSLETTER] Terdeteksi saluran dihapus/ditinggalkan (unfollow). Mencoba mengikuti kembali...`);
      try {
        ...
        console.error(`[NEWSLETTER] Gagal re-follow saluran:`, e);
      }
    }
  });
  ...
  sock.ev.on("chats.update", async (updates) => {
    ...
    for (const update of updates) {
      if (update.id === channelId && (update as any).newsletterRole === "GUEST") {
        console.log(`[NEWSLETTER] Perubahan status chat terdeteksi (GUEST). Auto-follow...`);
        try {
          if (instance.connectionStatus === "connected" && instance.activeSocket === sock) {
  ...
  sock.ev.on("newsletter-participants.update", async (update) => {
    const channelId = instance.config?.autoFollowChannelId || config.channel.autoFollowId;
    if (channelId && update.id === channelId) {
      if (update.action === 'demote' || update.action === 'leave' || update.new_role === 'guest') {
        console.log(`[NEWSLETTER] Perubahan status partisipan terdeteksi. Auto follow seketika...`);
        try {
          if (instance.connectionStatus === "connected" && instance.activeSocket === sock) {
            await sock.newsletterFollow(channelId).catch((e: any) => {
              if (!String(e).includes('unexpected response structure')) throw e;
  ...
  sock.ev.on("connection.update", async (update: any) => {
    ...
    if (connection === "connecting") {
      ...
      setTimeout(async () => {
        try {
          if (instance.connectionStatus !== "connected" || instance.activeSocket !== sock) {
            console.log(`[NEWSLETTER] Skipping follow: Connection changed or closed.`);
            return;
          }
          ...
          if (channelId) {
            ...
            console.log(`[NEWSLETTER] Status: Mengikuti saluran ${channelId}`);
          }
        } catch (e) {
          if (String(e).includes('Connection Closed')) {
            console.log(`[NEWSLETTER] Connection closed during follow attempt for ${channelId}`);
          } else {
            console.error(`[NEWSLETTER] Gagal mengikuti saluran:`, e);
          }
        }
      }, 10000);

      // Status Check Loop for Follow state
      if (!instance._channelFollowInterval && channelId) {
        instance._channelFollowInterval = setInterval(async () => {
          try {
            ...
            const metadata: any = await sock.newsletterMetadata("jid", channelId).catch((err) => {
                ...
                // Jika error mengandung indikasi kita bukan subscriber aktif (404, 403, 401, not authorized)
                if (
                   errMsg.includes('404') || 
                   errMsg.includes('403') || 
                   errMsg.includes('401') || 
            ...
            if (isGuest) {
               console.log(`[NEWSLETTER] Verifikasi Gagal: User bukan follower (${role || 'none'}). Mengikuti kembali...`);
               
               await sock.newsletterFollow(channelId).catch((e: any) => {
                 if (e.message && e.message.includes("Connection Closed")) return;
                 if (!String(e).includes('unexpected response structure')) {
                    console.error(`[NEWSLETTER] Error saat mencoba mengikuti:`, e.message || e);
                 }
               });
               
               // Tunggu sebentar lalu cek lagi secara internal untuk log sukses
               setTimeout(async () => {
                 if (instance.connectionStatus !== "connected" || instance.activeSocket !== sock) return;
                 const vMeta: any = await sock.newsletterMetadata("jid", channelId).catch(() => null);
                 if (vMeta?.viewer_metadata?.role && vMeta?.viewer_metadata?.role !== "GUEST") {
                    console.log(`[NEWSLETTER] Berhasil mengikuti kembali saluran ${channelId}!`);
                 }
               }, 3000);
            }
          } catch(e) {}
...
async function handleMessages(deviceId: string, chat: any) {
  ...
  try {
    ...
    for (const m of chat.messages) {
      ...
      const targetChannelId = deviceConfig?.autoFollowChannelId || deviceConfig?.channelId || config.channel.autoFollowId || config.channel.id;
      ...
      if (isNewsletter) {
        try {
          ...
          const targetPart = extractId(targetChannelId);

          if (!targetChannelId || chatPart === targetPart || chatId === targetChannelId) {
             const emojis = ["❤️", "🚀", "🔥", "✨", "👍", "👏", "🎉", "💯", "🤩", "🙌", "💥", "⚡", "🌈", "🌟", "💎", "🎯", "🦾", "💪", "🍭", "🍬", "🔥", "🧊"];
...
${v.passive_description || "-"}`;
    ...
    if (["ping"].includes(command || "")) {
      ...
      if (["ig", "instagram"].includes(command || "")) {
        ...
        await react("⏳");
        try {
          const res: any = await axios.get(`https://api.neoxr.eu/api/ig?url=${encodeURIComponent(url)}&apikey=${config.neoxrApiKey || 'Milik-Bot-OurinMD'}`);
...
_Mau langgar? Siap-siap di Kick!_`;
    ...
    if (["menu", "help"].includes(command || "")) {
        ...
        menu += `┃ ⏱️ *ᴜᴘᴛɪᴍᴇ:* ${fmtUp((Date.now() - ((global as any).startTime || Date.now())) / 1000)}\n`;
        menu += `┃ 📅 *ᴅᴀᴛᴇ:* ${moment().tz("Asia/Jakarta").format("DD/MM/YYYY")}\n`;
        menu += `╰━━━━━━━━━━━━┈.✦ ݁˖\n`;
        menu += `╭𐙚━〔 *OWNER* 〕━𐙚┈\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}ping\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}addowner\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}delowner\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}listowner\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}swgc\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}autoread\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}self\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}public\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}onlygc\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}onlypc\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}onlythisgrup\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}ban\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}unban\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}autosholat\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}listban\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}buatgrup\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}buatsaluran\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}upch\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}join\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}leave\`\n`;
        menu += `╰━━━━━━━━━━━━┈.✦ ݁˖\n`;
        menu += `╭𐙚━〔 *GROUP* 〕━𐙚┈\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}ht\` / \`${prefix}h\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}open\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}close\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}kick\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}promote\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}demote\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}afk\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}intro\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}setintro\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}resetintro\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}rulesgrup\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}setrulesgrup\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}resetrulesgrup\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}cekidgc\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}cekonline\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}linkgc\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}resetlinkgc\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}topchat\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}mulaiabsen\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}absen\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}cekabsen\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}hapusabsen\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}welcome\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}goodbye\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}antilinkgc\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}addantilink\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}delantilink\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}listantilink\`\n`;
        menu += `╰━━━━━━━━━━━━┈.✦ ݁˖\n`;
        menu += `╭𐙚━〔 *DOWNLOAD* 〕━𐙚┈\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}splay\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}ytplay\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}tt\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}ttmp3\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}ttplay\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}ig\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}fb\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}ytmp3\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}ytmp4\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}spdl\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}svdl\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}videy\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}terabox\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}tdl\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}ccdl\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}mfdl\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}githubdl\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}ytplayvid\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}playtiktok\`\n`;
        menu += `╰━━━━━━━━━━━━┈.✦ ݁˖\n`;
        menu += `╭𐙚━〔 *STICKER* 〕━𐙚┈\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}s\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}brat\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}bratvid\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}bratvid2\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}smeme\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}bratvermeil\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}bratanime\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}bratbahlil\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}bratgreen\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}bratcewek\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}bratsquidward\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}stickerpack\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}pinpack\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}bratpatrick\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}emojigif\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}emojimix\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}attp\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}qc\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}swm\`\n`;
        menu += `╰━━━━━━━━━━━━┈.✦ ݁˖\n`;
        menu += `╭𐙚━〔 *LOGO MAKER* 〕━𐙚┈\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}avengers\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}bear\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}blackpink\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}cartoon-graffiti\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}comic\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}glitch\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}mascot\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}naruto\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}pixel-glitch\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}pornhub\`\n`;
        menu += `╰━━━━━━━━━━━━┈.✦ ݁˖\n`;
        menu += `╭𐙚━〔 *RANDOM* 〕━𐙚┈\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}anime\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}blue-archive\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}cecan-china\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}cecan-indo\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}cecan-japan\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}cecan-korea\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}cecan-thailand\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}cecan-vietnam\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}loli\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}pap\`\n`;
        menu += `╰━━━━━━━━━━━━┈.✦ ݁˖\n`;
        menu += `╭𐙚━〔 *CANVAS* 〕━𐙚┈\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}iqc\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}nulis\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}fakeml\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}fakeff\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}fakebankjago\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}fakedev\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}fakestory\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}fakecall\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}fakedana\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}math\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}gura\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}pakustad\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}fakektp\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}susu\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}susutaro\`\n`;
        menu += `╰━━━━━━━━━━━━┈.✦ ݁˖\n`;
        menu += `╭𐙚━〔 *TOOLS* 〕━𐙚┈\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}rvo\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}translate\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}hd\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}hdvid\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}remini\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}blurface\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}removebg\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}deploy\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}toimg\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}tourl\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}kodepos\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}kalkulatormlbb\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}trackip\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}idch\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}spamngl\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}ipwho\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}lookup\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}qrcustom\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}readmore\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}pastebin\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}ptv\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}ssweb\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}web2zip\`\n`;
        menu += `╰━━━━━━━━━━━━┈.✦ ݁˖\n`;
        menu += `╭𐙚━〔 *STALKER* 〕━𐙚┈\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}igstalk\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}stalkml\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}stalkff\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}roblox\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}genshin\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}wastalk\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}ttstalk\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}gitstalk\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}pinstalk\`\n`;
        menu += `╰━━━━━━━━━━━━┈.✦ ݁˖\n`;
        menu += `╭𐙚━〔 *AI* 〕━𐙚┈\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}to3d\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}tochibi\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}toblack\`\n`;
        menu += `╰━━━━━━━━━━━━┈.✦ ݁˖\n`;
        menu += `╭𐙚━〔 *ISLAMIC* 〕━𐙚┈\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}quran\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}murrotal\`\n`;
        menu += `╰━━━━━━━━━━━━┈.✦ ݁˖\n`;
        menu += `╭𐙚━〔 *NSFW* 〕━𐙚┈\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}hentai\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}kasedaiki\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}gangbang\`\n`;
        menu += `╰━━━━━━━━━━━━┈.✦ ݁˖\n`;
        menu += `╭𐙚━〔 *FUN* 〕━𐙚┈\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}akankah\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}apakah\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}bagaimana\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}berapa\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}bisakah\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}cekkhodam\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}cekpacar\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}coba\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}confess\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}dimana\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}gay\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}haruskah\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}jodoh\`\n`;
        menu += `╰━━━━━━━━━━━━┈.✦ ݁˖\n`;
        menu += `╭𐙚━〔 *GAME* 〕━𐙚┈\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}tebakbendera\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}tebakgambar\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}lengkapikalimat\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}tebakkata\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}tekateki\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}asahotak\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}tebaklagu\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}tebakheroml\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}tebaklogo\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}tebakgame\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}tebakkalimat\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}cerdascermat\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}susunkata\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}siapakahaku\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}caklontong\`\n`;
        menu += `╰━━━━━━━━━━━━┈.✦ ݁˖\n`;
        menu += `╭𐙚━〔 *OTHER* 〕━𐙚┈\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}gempa\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}cuaca\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}npm\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}pins\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}buildml\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}infotourney\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}bluearchive-char\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}berita\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}cnn\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}cnbc\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}antara\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}sindonews\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}jadwaltv\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}jadwalbola\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}lirik\`\n`;
        menu += `╰━━━━━━━━━━━━┈.✦ ݁˖\n`;
        menu += `╭〔 *SUPORT DEVELOPER* 〕┈\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}qris\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}saweria\`\n`;
        menu += `┃ ᡣ𐭩 \`${prefix}tako\`\n`;
        menu += `╰━━━━━━━━━━━━┈`;
        ...
        await sock.sendMessage(chatId, {
          text: menu,
          mentions: [sender],
          contextInfo: getContextInfo(deviceConfig, m, null, true, true, false, "https://bot.cmnty.qzz.io"),
        }, { quoted: getVerifiedQuoted(deviceConfig) as any });
        return;
      }
    ...
    app.get("/api/config", (req, res) => {
      try {
        ...
        res.json({
          ...
          ownerNumber,
          stickerPack: deviceConfig.stickerPack || "CMNTY-BOT",
          stickerAuthor: deviceConfig.stickerAuthor || "@OJI",
          ...
          thumbnailUrl: globalConf.thumbnailUrl || "https://c.termai.cc/i180/5n5c.jpg",
