...
const DEFAULT_METADATA = {
    packname: 'CMNTY-BOT',
    author: 'Bot',
    packId: 'com.cmnty.sticker.' + Date.now(),
    emojis: ['🤖']
};

function createExif(options: any = {}) {
...
function imageToWebpFFmpeg(buffer: Buffer): Promise<Buffer> {
...
function videoToWebpFFmpeg(buffer: Buffer, options: any = {}): Promise<Buffer> {
    return new Promise((resolve, reject) => {
        const tmpDir = getTempDir();
...
function replaceExifInWebp(webpBuffer: Buffer, newExif: Buffer) {
...
function addExifToWebpFallback(webpBuffer: Buffer, options: any = {}) {
...
async function addExifToWebp(webpBuffer: Buffer, options: any = {}) {
    await initWebpmux();
...
async function createStickerFromImage(imageBuffer: Buffer, options: any = {}) {
    const webpBuffer = await imageToWebpFFmpeg(imageBuffer);
    return await addExifToWebp(webpBuffer, options);
}

async function createStickerFromVideo(videoBuffer: Buffer, options: any = {}) {
...
function readExifFromWebp(webpBuffer: Buffer) {
    const exifIndex = webpBuffer.indexOf(Buffer.from('EXIF'));
...
async function isAnimatedWebp(buffer: Buffer) {
